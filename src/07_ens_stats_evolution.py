...
for ii, data in enumerate([data_2D,data_3D]):
    ...
    ax1.plot(nn,cv_por_theory,'k-',zorder = 2)      #,label='Porosity'
    ax1.plot(nn,por_level*np.ones_like(nn),c='0.6',ls='--',zorder = 0)  
    ax1.plot(nn,cv_log_ta_theory,'k:',zorder = 3)   #,label='Transport Ability'
    ...
    ax2.plot(nd,cv_por_theory,'k-',zorder = 2)      #,label='Porosity'
    ax2.plot(nd,por_level*np.ones_like(nn),c='0.6',ls='--',zorder = 0)  
    ax2.plot(nd,cv_log_ta_theory,'k:',zorder = 3)   #,label='Transport Ability'
...
ax1.set_xlabel(r'domain length $r$ [$\mu$m]',fontsize=textsize) # plt.xlabel('Connected Porosity')
...
ax2.set_xlabel(r'Number of network elements $N_r = (r/2)^d$',fontsize=textsize) # plt.xlabel('Connected Porosity')        
